...
const entryUrl =
  'https://www.immobiliare.it/search-list/?fkRegione=sar&idProvincia=SS&idNazione=IT&idContratto=1&idCategoria=1&idTipologia%5B0%5D=7&idTipologia%5B1%5D=11&idTipologia%5B2%5D=12&tipoProprieta=1&giardino%5B0%5D=10&criterio=prezzo&ordine=asc&__lang=it'
...
const getAnnouncementFeatures = async (page: Page, link: string) => {
  await page.goto(link)
  await page.waitForSelector('.in-realEstateFeatures__list')

  const announcementFeatures = await page.evaluate(() => {
    const labels = Array.from(
      document.querySelectorAll('.in-realEstateFeatures__title'),
      (el) => el.textContent
    )
    const values = Array.from(
      document.querySelectorAll('.in-realEstateFeatures__value'),
...
export const harvest = async () => {
  ...
  try {
    ...
    const paginationContent = await page.evaluate(() =>
      Array.from(
        document.querySelectorAll('.in-pagination__item--disabled'),
        (element) => element.textContent
      )
    )
    const lastPageNumber = paginationContent.filter(
      (text) => text && !isNaN(parseInt(text))
    )
    ...
    while (pageCounter.getValue() <= lastPage) {
      ...
      const hrefs = await page.evaluate(() =>
        Array.from(
          document.querySelectorAll('.in-card__title'),
          // @ts-ignore
          (element) => element.href
        )
      )
      const links: string[] = [...hrefs.filter(Boolean)]
      ...
      for (const link of links) {
        ...
        console.log(
          `Harvester: harvested ${linkCount} on ${pageCounterValue}, ${pagesLeft} pages left`
        )
